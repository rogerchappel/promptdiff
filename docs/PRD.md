...
PromptDiff is a local-first TypeScript CLI that compares prompt/template revisions and highlights risky instruction, tool, and output-contract changes. It is built for developers and agentic coding loops that need deterministic evidence, tidy fixtures, and no surprise network calls. It treats prompts like code: diffable, reviewable, and boring in the best way.
...
- Agent builders reviewing prompt changes before they break production workflows
...
```bash
promptdiff compare prompts/old.md prompts/new.md --out prompt-risk.md\npromptdiff check prompts/*.md --rules examples/rules.json
```
...
- GitHub repository is public under `rogerchappel/promptdiff` with useful description and topics.
